...
from oas_erf.util.eusaar_data import p_gen, p_distc, station_codes, subs_codes, years_codes
from oas_erf.constants import path_eusaar_outdata
...
def get_distc_dataframe(station, year='BOTH', subs='TOT'):
    rows= ['%.0fth percentile' % p for p in percs_in_eusaar_files]#, '16th percentile', '50th percentile','84th percentile']
    diam = get_diameter_sized()
    p = p_distc + station +'_'+subs+'_DIST_'+year+'.dat'
    m = np.loadtxt(p)
...
def get_distc_xarray_all(from_nc=True):
    ...
    if from_nc and os.path.isfile(savepath_distc_vars):
        return xr.open_dataset(savepath_distc_vars)#.rename({'':'dN/dlog10(d$_p$)'})
    ls=[]
    ...
    return da_c
# %%
#da_c = get_distc_xarray_all(from_nc=False)