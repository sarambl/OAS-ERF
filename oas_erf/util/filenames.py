...
def get_filename_ng_field(var, model, case, from_time, to_time,
    ...
    try:
        ...
        pass
    filename_m = path_save_ng + '/%s/%s/%s_%s_%s_%s-%s.nc' % (model, case, var, model, case, from_time, to_time)
    filename_m = filename_m.replace(" ", "_")
    return filename_m
...
def filename_map_avg(model, case, var,
    ...
    :return:
    """
    filen = path_to_data + '/%s/%s/%s_%s-%s' % (model, case, var, startyear, endyear)
...
def filename_area_avg(var, model, case, area,
    ...
    :return:
    """
    filen = path_to_data + f'/{model}/{case}/{area}/{var}_{startyear}-{endyear}'  # % (model, case, area, var, startyear, endyear)
    if not lev_was_coord:
...
def filename_profile_avg_allvars(model, case, area,
    ...
    :return:
    """
    filen = path_to_data + '/%s/%s/%s/%.0f-%.0f' % (model, case, area, startyear, endyear)
    if not pres_adj:
        filen = filen + 'not_pres_adj.nc'
    else:
        filen = filen + '.nc'
...
def filename_profile_avg(var, case, area,
    ...
    :return:
    """
    filen = path_to_data + '/%s/%s/%s/%s_%s-%s' % (model, case, area, var, from_time, to_time)
    if time_mask is not None:
        filen = filen + '_%s' % time_mask
