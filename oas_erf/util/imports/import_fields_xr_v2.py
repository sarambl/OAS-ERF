...
from oas_erf.data_info.variable_info import import_constants as import_constants_list
from oas_erf.util.filenames import get_filename_constants
from oas_erf import constants
from oas_erf.util.imports.fix_xa_dataset_v2 import xr_fix
from oas_erf.constants import latlon_path
...
def xr_import_NorESM(case, varlist, from_time, to_time, path=constants.get_input_datapath(),
    ...
    if varlist is not None:
        ...
        drop_list = list((set(dummy.data_vars.keys()) - set(var_names_mod)) - set(
            oas_erf.data_info.variable_info.import_always_include))
        dummy.close()
    else:
        drop_list = None
    # Read the files:
    ...
    attrs_ds = dict(raw_data_path=path,
