...
import oas_erf.util.naming_conventions.var_info
# import analysis_tools.fix_xa_dataset as fix_xa_dataset
# from oas_erf.util.plot import  plot_settings
from oas_erf.data_info import get_nice_name_case, get_area_specs
from oas_erf.util.slice_average.area_mod import lon180_2lon360
from oas_erf.util.slice_average.avg_pkg import average_model_var  # , get_lat_wgts_matrix, masked_average
...
def plot_map(var, case, cases_dic, figsize=None,
    ...
    label = get_fancy_var_name(var) + ' [%s]' % oas_erf.util.naming_conventions.var_info.get_fancy_unit_xr(plt_var,
                                                                                                                var)
    ...
    unit = oas_erf.util.naming_conventions.var_info.get_fancy_unit_xr(plt_var, var)
...
def plot_map_diff(var, case_ctrl, case_oth, cases_dic, figsize=None, relative=False,tit_ext='',
                  contourf=False,
                  ax=None,
                  cmap_diff='RdBu_r',
                  cbar_orientation='vertical',
    ...
    if relative:
        ...
        func = fdifference
        unit_diff = ' [%s]' % oas_erf.util.naming_conventions.var_info.get_fancy_unit_xr(ctrl_da, var)
...
def make_box(area, ax, facecolor=None, alpha=0., edgecolor='#66a61e', kwargs=dict()):
    # %%
...
def plot_map_diff_2case(var, case1, case2, cases_dic, relative=False, ax=None, vmax=None, vmin=None,
                        kwargs_diff={}, cmap_diff='RdBu_r', figsize=None, cbar_eq = False):
    if vmin is not None:
...
def plot_map_diff_only(var, cases, cases_dic, relative=False, figsize=[4, 3], cbar_equal=True,
    ...
    for to_case, ax in zip(cases[1:], axs):
        ...
        ax, kwargs_diff = plot_map_diff(var, _ctrl_case, _to_case,cases_dic,relative=relative,
                       ax=ax,
                      cbar_orientation=cbar_orientation,**kwargs_diff)
    if tight_layout:
        plt.tight_layout()
    return axs, kwargs_diff.copy()


def plot_map_abs_only(var, cases, cases_dic, relative=False, figsize=[4, 3], cbar_equal=True,
                      kwargs_abs={}, axs=None, cmap_abs='Reds', cbar_loc='side', nr_rows=1):
    """
    Plots absolute absolute difference for cases in cases
    :param var: variable to plot
    :param cases: cases to plot
...
def get_avg_diff(case_oth, case_ctrl, case_dic, relative, var):
    ...
    if relative:
        ...
        unit = oas_erf.util.naming_conventions.var_info.get_fancy_unit_xr(case_dic[case_ctrl][var], var)
