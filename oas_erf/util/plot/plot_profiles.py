...
def plot_profile(da, ax, xscale='log', yscale='log',
    ...
    if pressure_coords:
        ax.set_ylabel('Pressure [hPa]')
    var = da.name
    xlabel = get_fancy_var_name(var) + ' [%s]' % oas_erf.util.naming_conventions.var_info.get_fancy_unit_xr(da,
                                                                                                                 var)
...
def plot_var_multicase(ax, var, nested_cases, cases, pltkwargs=None, area='', area_in_label=False):
    ...
    xlab = '%s [%s]' % (
    oas_erf.util.naming_conventions.var_info.get_fancy_var_name_xr(nested_cases[case][var], var),
    oas_erf.util.naming_conventions.var_info.get_fancy_unit_xr(nested_cases[case][var], var))
    ax.set_xlabel(xlab)
